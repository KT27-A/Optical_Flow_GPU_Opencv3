...
static void convertFlowToImage(const Mat &flowIn, Mat &flowOut, Mat &flowOut_cal,
	...
	for (int i = 0; i < flowIn.rows; ++i) {
		for (int j = 0; j < flowIn.cols; ++j) {
			...
			flowOut_cal.at<float>(i,j) = CAST(x, lowerBound, higherBound);
		}
	}
	#undef CAST
}
...
int main( int argc, char *argv[] )
{
	...
	for (; (dirIt.hasNext()); )
	{
		...
		QString file = dirIt.fileName();
			...
			QString out_folder_flow = QString::fromStdString(out_path + "flow/" + fName);
			...
			while( frame1.empty() == false )
			{
				...
				if( WRITEOUT_IMGS == true &&  nframes >= 1 )
				{
					...
					cv::Mat img_u_cal, img_v_cal;

					convertFlowToImage(imgU, img_u, img_u_cal, min_u_f, max_u_f);
					convertFlowToImage(imgV, img_v, img_v_cal, min_v_f, max_v_f);

					// multiply(img_u, img_u, planes[0]);
					// multiply(img_v, img_v, planes[1]);
					pow(img_u, 2.0f, planes[0]);
					pow(img_v, 2.0f, planes[1]);
					...
					for (int i = 0; i < img_u.rows; ++i) {
						for (int j = 0; j < img_u.cols; ++j) {
							...
							optflow.at<Vec3b>(i,j)[2] = mag.at<float>(i,j);
						}
					}		
